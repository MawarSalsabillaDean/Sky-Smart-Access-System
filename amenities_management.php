...
// Search query
$search = $_GET['search'] ?? ''; // The null coalescing operator '??' is used to provide a default value if $_GET['search'] is not set

// SQL to fetch the amenities usage history
...
if (!empty($search)) {
    $sql .= " WHERE ra.full_name LIKE :search OR ra.phone_number LIKE :search OR ra.unit_number LIKE :search OR au.amenity_name LIKE :search";
}
    ...
    <div class="sidebar">
        ...
        <div class="profile_content">
            <div class="profile">
                ...
                <button type="button" name="logout" id="log_out" class='bx bx-log-out' onclick="location.href='amenities_management.php?logout=true';"></button>
                </div>
        </div>
    </div>
            ...
            <tbody>
                <?php while ($row = $stmt->fetch(PDO::FETCH_ASSOC)): ?>
                    <tr>
                        ...
                        <td data-title="End Time"><?= $row['end_time'] ? formatDate($row['end_time']) : 'Not Available' ?></td>
