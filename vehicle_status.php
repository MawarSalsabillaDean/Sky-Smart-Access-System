...
try {
    ...
    $stmt = $pdo->prepare("SELECT brand, model, vehicle_registration_number, parking_spot, registration_timestamp, status, comments FROM vehicles WHERE resident_id = ? ORDER BY registration_timestamp DESC");
...
<head>
    ...
    <link rel="stylesheet" href="vehicle_status.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>

    <title>Vehicle Tracking Status</title>
</head>
<body>
    <!-- sidebar -->
    <div class="sidebar">
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                    <td data-label="Brand"><?= htmlspecialchars($request['brand']) ?></td>
                    <td data-label="Model"><?= htmlspecialchars($request['model']) ?></td>
                    <td data-label="Plate Number"><?= htmlspecialchars($request['vehicle_registration_number']) ?></td>
                    <td data-label="Parking Spot"><?= !empty($request['parking_spot']) ? htmlspecialchars($request['parking_spot']) : '-' ?></td>
